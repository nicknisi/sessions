...
//
// The watermark is GLOBAL, keyed by file path, while `shards mine` is scoped by
// default. The two meet in one place worth knowing about: `/weekly-summary` runs
// `shards mine --all --since-last` (plugin/skills/weekly-summary/SKILL.md), which
...
//
// The table lives in shards.db, not index.db: the index is disposable by design
 ...
 * src/shards/cli.ts's `persistMine`.
 *
 * Pass only the entries that were actually IN SCOPE for this mine. `shards mine`
 ...
 *
 * `mined_at` is a clock read, which every other function under src/shards/ takes as
