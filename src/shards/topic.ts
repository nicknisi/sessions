// Topic matching: how relevant a shard is to what the agent says it is about to do.
//
// This module is PURE and imports nothing — no store, no index, no clock. That is a
// constraint, not an accident: src/shards/topic.test.ts drives it with plain strings
// and needs no tmpdir harness, and anything that reaches into src/shards/store.ts or
// src/wrapped/content.ts transitively opens a database on import. Same rule
// src/shards/quorum.ts ships under.
//
// What this is NOT: a ranker. See the comment on `matchTopic`.

/**
 * Below this score, a shard that is not marked always-on is dropped from the active
 * set for a given topic.
 *
 * The score is `|topic ∩ shard| / |topicTokens|`, so the SHARD's length never enters
 ...
 *
 * This number is a guess made before there was enough shard volume to tune anything
...
const STOPWORDS = new Set(
  (
    ...
    // negated topic carries a spurious content token (src/shards/mine.ts:39-42).
 ...
 * src/shards/mine.ts:39-42). That looks like a bug and is not — matching the index's
 ...
 * is an asserted success criterion. It was written before there was enough shard volume
 * to tune anything more sophisticated against, and a bespoke scorer fitted to fifteen
 * shards would be fitted to noise. Replace the body, not the signature, once real usage
 ...
 * `NaN >= TOPIC_THRESHOLD` is false, so every conditional shard would vanish. A silent
...
export function matchTopic(shardText: string, topic: string): number {
  ...
  const shardTokens = tokenize(shardText);
  if (shardTokens.size === 0) return 0;
  let hits = 0;
  for (const token of topicTokens) {
    if (shardTokens.has(token)) hits++;
  }
