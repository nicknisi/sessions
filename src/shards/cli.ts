...
Usage:
  ...
  sessions shards mine --since-last  Mine only what changed since the last mine
  sessions shards pending          Count and preview candidates awaiting triage
...
/**
 * Parse `shards pending [--json]`. Throws `UsageError`; never exits.
 *
 ...
 *
 * Pure over `listShards`'s output so the projection is testable without a CLI.
 * `listShards` already returns `ORDER BY id`, which is arbitrary but stable — the
 ...
 */
export function pendingBatch(candidates: ShardRecord[]): PendingBatch {
  return {
...
async function runPending(argv: string[]): Promise<void> {
  ...
  const batch = pendingBatch(listShards({ state: 'candidate' }));
...
export function persistMine(
  ...
  upsert: (records: ShardRecord[]) => void = upsertCandidates,
): void {
  upsert(mined);
