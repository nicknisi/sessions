...
import { getShardsDb, listShards, upsertCandidates } from './store';
import { SHARD_SCHEMA_VERSION, type PortableShard, type ShardBundle, type ShardRecord } from './types';
...
function record(text: string, over: Partial<ShardRecord> = {}): ShardRecord {
  return {
    ...buildRecord({
      text,
...
function portable(text: string, over: Partial<Omit<PortableShard, 'id' | 'text'>> = {}): PortableShard {
...
describe('toPortable', () => {
  test('exports approved records only — a candidate is unreviewed model output', () => {
    ...
    expect(bundle.shards.map((s) => s.text)).toEqual([APPROVED.text]);
  });
  ...
  test('carries no local paths — session paths and the repo container both', () => {
    ...
    expect(bundle.shards[0]!.scope).toEqual({ type: 'repo', key: '' });
  });
  ...
  test('two exports of an unchanged set are byte-identical, whatever the row order', () => {
    ...
    const ids = toPortable(more, '2026-06-01').shards.map((s) => s.id);
...
describe('fromPortable', () => {
  ...
  test('round-trips an exported bundle back to the same shard set', () => {
    const parsed = fromPortable(JSON.parse(JSON.stringify(bundle)));
    expect(JSON.stringify(parsed)).toBe(JSON.stringify(bundle.shards));
  });
  ...
  test('throws on a version mismatch rather than best-effort parsing', () => {
    ...
    expect(() => fromPortable({ ...bundle, shards: [{ ...bundle.shards[0]!, v: 99 }] })).toThrow(PortableFormatError);
  });
  ...
  test('throws on a malformed bundle', () => {
    ...
    expect(() => fromPortable({ v: SHARD_SCHEMA_VERSION, shards: [] })).toThrow(PortableFormatError); // no exportedAt
    const { author: _dropped, ...missingAuthor } = bundle.shards[0]!;
    expect(() => fromPortable({ ...bundle, shards: [missingAuthor] })).toThrow(PortableFormatError);
  });
  ...
  test('rejects an unknown key instead of silently stripping it', () => {
    ...
    expect(() => fromPortable({ ...bundle, shards: [{ ...bundle.shards[0]!, always: true }] })).toThrow(
      PortableFormatError,
    );
  });
  ...
  test('recomputes the content-addressed id and rejects one that does not match its text', () => {
    ...
    const forged = { ...bundle, shards: [{ ...bundle.shards[0]!, text: 'Always do something else entirely, please' }] };
    expect(() => fromPortable(forged)).toThrow(/does not match its text/);
    const badShape = { ...bundle, shards: [{ ...bundle.shards[0]!, id: 'sha256:nothex' }] };
    expect(() => fromPortable(badShape)).toThrow(PortableFormatError);
  });

  test('rejects text outside the band the local mine enforces', () => {
    // Import is otherwise the one unbounded path into the store, and an approved shard
    // is injected into every agent task through get_shards.
    const long = 'Always '.repeat(60);
    expect(() => fromPortable({ ...bundle, shards: [portable(long)] })).toThrow(PortableFormatError);
    expect(() => fromPortable({ ...bundle, shards: [portable('too short')] })).toThrow(PortableFormatError);
  });
  ...
  test('accepts the empty date range the local pipeline itself produces', () => {
    ...
    expect(fromPortable({ ...bundle, shards: [undated] })).toHaveLength(1);
  });
});
...
describe('toRecord', () => {
  ...
  test('lands an imported shard as a candidate — importing is not consent', () => {
...
beforeAll(() => {
  tmp = makeTmp('shards-export');
  setShardEnv(tmp);
  closeDatabases();
});

// Shared module instances across a `bun test` run mean another file's env leaks in
// unless both are re-asserted per test (src/shards/fixtures.ts).
beforeEach(() => {
  setShardEnv(tmp);
  closeDatabases();
...
describe('shards export', () => {
  test('prints an envelope of approved shards on stdout and nothing local in it', async () => {
    const { stdout, stderr } = await capture(['export']);
    const bundle = JSON.parse(stdout) as ShardBundle;
    expect(bundle.v).toBe(SHARD_SCHEMA_VERSION);
    expect(bundle.shards.map((s) => s.text)).toEqual([APPROVED.text]);
    expect(stdout).not.toContain('testuser');
    expect(stdout).not.toContain('"state"');
    expect(stderr).toContain('1 approved shard exported');
  });
  ...
  test('--out writes the file and reports it on stderr, leaving stdout empty', async () => {
    ...
    expect((JSON.parse(readFileSync(out, 'utf-8')) as ShardBundle).shards).toHaveLength(1);
  });
...
describe('shards import', () => {
  test('importing your own export is a no-op, evidence included', async () => {
    ...
    const stored = listShards().find((r) => r.id === APPROVED.id)!;
  ...
  test('a foreign shard lands as a candidate, never as approved', async () => {
    ...
    const stored = listShards().find((r) => r.id === peer.id)!;
    expect(stored.state).toBe('candidate');
    expect(stored.author).toBe('peer@example.com');
    // Scope arrives keyless, and retrieve.ts skips a keyless repo shard rather than
    // matching every cwd — an imported repo shard is inert until it is re-derived.
  ...
  test('an id already rejected locally stays rejected', async () => {
    ...
    expect(listShards().find((r) => r.id === REJECTED.id)!.state).toBe('rejected');
  });
  ...
  test('duplicate ids inside one bundle collapse instead of racing each other', async () => {
    ...
    await Bun.write(
      out,
      JSON.stringify({ v: SHARD_SCHEMA_VERSION, exportedAt: '2026-06-01', shards: [peer, peer, peer] }),
    );

    const { stderr } = await capture(['import', out]);
    expect(stderr).toContain('1 imported');
    expect(listShards().filter((r) => r.id === peer.id)).toHaveLength(1);
  });
