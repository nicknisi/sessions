...
import { getPersistedStates, getShardsDb, listShards, setState, upsertCandidates } from './store';
...
const REPO_B: ShardScope = { type: 'repo', key: '/repos/other' };
...
describe('listShards', () => {
  test('no filter returns every shard, ordered by id', () => {
    const all = listShards();
  ...
  test('filters by each state', () => {
    expect(texts(listShards({ state: 'approved' }))).toEqual([APPROVED.text]);
    expect(texts(listShards({ state: 'rejected' }))).toEqual([REJECTED.text]);
    expect(texts(listShards({ state: 'candidate' }))).toEqual([CANDIDATE.text]);

    const snoozed = listShards({ state: 'snoozed' });
  ...
  test('filters by a repo scope, matching type AND key', () => {
    expect(texts(listShards({ scope: REPO_A }))).toEqual([APPROVED.text, REJECTED.text].sort());
    expect(texts(listShards({ scope: REPO_B }))).toEqual([SNOOZED.text]);
    expect(listShards({ scope: { type: 'repo', key: '/repos/nothing' } })).toEqual([]);
  });
  ...
  test('filters by workflow scope, whose key is the empty string', () => {
    ...
    // through to every repo shard.
    expect(texts(listShards({ scope: WORKFLOW }))).toEqual([CANDIDATE.text]);
  });

  test('state and scope compose', () => {
    expect(texts(listShards({ state: 'rejected', scope: REPO_A }))).toEqual([REJECTED.text]);
    expect(listShards({ state: 'approved', scope: REPO_B })).toEqual([]);
  });
});
