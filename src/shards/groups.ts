...
// workflow rules. Phase 1's spread heuristic (src/shards/mine.ts:151-156) cannot derive
 ...
 * `/tmp/x/authkit-session/packages/core` — and `activeShardsFor` passes whatever
 ...
 * src/shards/retrieve.ts:24-32. There, a stored repo path lands in the pattern position
 ...
 * `activeShardsFor` promises byte-identical output for identical inputs.
 */
export function groupsFor(container: string, config: GroupConfig): string[] {
  ...
  for (const [name, patterns] of Object.entries(config.groups)) {
    // An unnamed group cannot be referenced by a shard's scope key, and an empty key is
    // skipped on retrieval anyway — matching it would attach a group to every shard.
    if (!name) continue;
 ...
 * deliberate:
 *
 *  - It writes NOTHING to stderr. This runs inside `get_shards`, and the MCP server
 ...
 *
 * Re-read on every call rather than memoized. `activeShardsFor` runs once per tool
 * invocation and only reaches this when a group-scoped shard exists, so the cost is one
