...
import { mineRestriction, persistMine, runShards } from './cli';
import { FILE_CHUNK, mine } from './mine';
import { getShardsDb, listShards, setState } from './store';
...
// (src/shards/mine.perf.test.ts:55 does the same).
...
function batchOf(result: { stdout: string }): ShardRecord[] {
  return JSON.parse(result.stdout) as ShardRecord[];
}
...
beforeAll(() => {
  ...
  spreadTmp = makeTmp('shards-stream-spread');
...
describe('mine --since-last', () => {
  ...
  test('deleting a watermark row makes that session changed again', async () => {
    await capture(['mine', '--repo', repo, '--since-last']);
    getShardsDb().run('DELETE FROM mine_watermark WHERE file_path = ?', [paths.s1]);

    expect(texts(batchOf(await capture(['mine', '--repo', repo, '--since-last'])))).toEqual([S1]);
  });

  test('a rediscovered phrasing on a rejected shard stays rejected', async () => {
  ...
  test('scope is derived over the union, so a third repo widens an existing shard', async () => {
    setShardEnv(spreadTmp);
    closeDatabases();
    try {
      const first = batchOf(await capture(['mine', '--all', '--since-last']));
      // Two containers is ambiguous and resolves toward `repo` (src/shards/mine.ts).
...
describe('distinctPhrasings stays 1, and snooze-resurface therefore cannot fire', () => {
  ...
  // amendment and Open Item in docs/ideation/context-shards/spec-phase-6.md: the spec's
  ...
  test('a snoozed candidate stays hidden even once a paraphrase arrives and the date passes', async () => {
    ...
    try {
      ...
      expect(listShards().find((r) => r.id === target.id)!.state).toBe('snoozed');
