...
function record(text: string, distinctPhrasings: number): ShardRecord {
  return buildRecord({
    text,
...
describe('shouldResurface', () => {
  const snoozed: ShardRecord = { ...SNOOZED, state: 'snoozed', snoozedUntil: '2026-01-31' };
...
describe('write-back', () => {
  test('snooze records both the state and the expiry the filter reads', () => {
    ...
    const stored = listShards({ state: 'snoozed' })[0]!;
  ...
  test('approve on a rejected id succeeds — a user may change their mind', () => {
    reject(REJECTED.id);
    expect(listShards({ state: 'rejected' }).map((r) => r.id)).toEqual([REJECTED.id]);
    approve(REJECTED.id);
    expect(listShards({ state: 'rejected' })).toHaveLength(0);
    expect(listShards({ state: 'approved' }).map((r) => r.id)).toEqual([REJECTED.id]);
  });

  test('approve and reject clear a pending snooze', () => {
    snooze(SNOOZED.id, '2026-01-01');
    approve(SNOOZED.id);
    expect(listShards({ state: 'approved' })[0]!.snoozedUntil).toBeNull();
  });
});
...
describe('dropSuppressed', () => {
  test('a rejected candidate leaves the batch entirely', () => {
    reject(REJECTED.id);
    const batch = dropSuppressed([REJECTED, FRESH], suppressedShards(), '2030-01-01');
    expect(batch.map((r) => r.id)).toEqual([FRESH.id]);
  });

  test('an untriaged candidate always passes through', () => {
    expect(dropSuppressed([FRESH], suppressedShards(), '2030-01-01')).toEqual([FRESH]);
  });

  test('an approved shard stays in the batch — the pipe mirrors the table', () => {
    approve(FRESH.id);
    expect(dropSuppressed([FRESH], suppressedShards(), '2030-01-01').map((r) => r.id)).toEqual([FRESH.id]);
  });
  ...
  test('suppressedShards carries evidence, which getPersistedStates cannot', () => {
    setState(SNOOZED.id, 'snoozed', '2026-01-31');
    const stored = suppressedShards().get(SNOOZED.id)!;
