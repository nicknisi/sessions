...
4. **Read what is already binding.** `sessions memory documented --json` lists every statement an agent working here is _already_ told — the global `CLAUDE.md`, a repo `CLAUDE.md` or `AGENTS.md`, and Claude Code's own per-project memory store. Read it before the rubric, because it decides the rubric's first question.
