...
5. **Persist.** One command per decision, using the cluster's `id`:
   - `sessions shards approve <id>` (add `--always-on` for a standing constraint)
   - `sessions shards reject <id>`
   - `sessions shards snooze <id>`
